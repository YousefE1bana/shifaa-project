...
END $$;
DO $$ BEGIN
 IF (SELECT count(DISTINCT role_code) FROM identity.role_permissions)<>5 THEN RAISE EXCEPTION 'only the five canonical admin roles should be seeded'; END IF;
 ...
 IF (SELECT count(*) FROM identity.role_permissions)<>24 THEN RAISE EXCEPTION 'expected exact permissions through feature 008'; END IF;
END $$;
