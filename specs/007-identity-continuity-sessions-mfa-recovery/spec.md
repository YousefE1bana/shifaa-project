...
| `completeRecovery` — `POST /auth/recovery/{caseId}/complete` | opaque case holder with approved proof/factor evidence                                                                                                                                                           | evidence + new credential → `200` new ordinary/restricted session; when a new repeated proof is required, `proof_required` plus a short-lived Recovery Proof Grant for the existing proof-case creation path | expired/replayed/wrong proof/grant/race; all old sessions revoked before ordinary access                                    |

The Recovery Proof Grant reconciliation does not create a ninth Feature 007 operation. The opaque
grant is bound to the recovery case, subject person, `account_recovery_reproof` purpose, and expiry;
it is accepted only by the existing `createIdentityProof` path to create and link one verification
case after recovery intake. It is not an access JWT or session, grants no reads or other mutations,
does not expand the four-operation restricted-session allowlist, and fails closed for expiry, replay,
or binding mismatch. `completeRecovery` is retried with the linked approved `verificationCaseId`
without redeeming the recovery OTP again.
