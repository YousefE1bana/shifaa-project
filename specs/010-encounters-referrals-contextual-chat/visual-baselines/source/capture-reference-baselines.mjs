...
const galleryPath = path.join(root, 'review-gallery.html');
...
const workspaceRoot = execFileSync('git', ['-C', root, 'rev-parse', '--show-toplevel'], { encoding: 'utf8' }).trim();
const workspacePrefix = path.resolve(workspaceRoot) + path.sep;
const sourceRelative = path.relative(workspaceRoot, sourcePath).split(path.sep).join('/');
...
const startSourceServer = async port => {
  const server = http.createServer((req, res) => {
    try {
      ...
      const file = path.resolve(workspaceRoot, `.${pathname}`);
      if (!file.startsWith(workspacePrefix)) throw new Error('outside root');
...
const profileRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'shifaa-f010-p0-cdp-'));
...
try {
  ...
  if (smoke) {
    const smokeBaseline = arg('--baseline') || 'F010-P0-PAT-RECORDS-001';
    const smokeLocale = arg('--locale') || 'ar-EG';
    const smokeViewport = arg('--viewport') || '360x800';
    const smokeState = arg('--state') || 'pending';
    const job = jobs.find(item => item.baselineId === smokeBaseline && item.locale === smokeLocale && item.viewport === smokeViewport && item.state === smokeState);
    if (!job) throw new Error(`Unknown smoke inventory combination: ${smokeBaseline}/${smokeLocale}/${smokeViewport}/${smokeState}`);
    ...
    const expectedPngs = new Set(jobs.map(job => path.resolve(job.absolute)));
    const pruneRemovedStates = directory => {
      for (const item of fs.readdirSync(directory, { withFileTypes: true })) {
        const absolute = path.resolve(directory, item.name);
        if (!absolute.startsWith(path.resolve(referenceRoot) + path.sep)) throw new Error(`Unsafe reference path: ${absolute}`);
        if (item.isDirectory()) pruneRemovedStates(absolute);
        else if (item.isFile() && item.name.endsWith('.png') && !expectedPngs.has(absolute)) fs.unlinkSync(absolute);
      }
    };
    pruneRemovedStates(referenceRoot);
    const entries = jobs.map(job => ({ baselineId: job.baselineId, sourceNode: `${inventory.sourceVersion}#${job.baselineId}--${job.state}`, sourceVersion: inventory.sourceVersion, app: job.app, route: job.route, locale: job.locale, viewport: job.viewport, state: job.state, fixtureId: 'F010-SYNTHETIC-VISUAL-001', referenceArtifact: job.relative, sha256: sha256(job.absolute) }));
    ...
    const manifestScript = /<script id="manifest-data" type="application\/json">[\s\S]*?<\/script>/;
    if (!manifestScript.test(gallery)) throw new Error('Gallery manifest block missing');
    fs.writeFileSync(galleryPath, gallery.replace(manifestScript, () => `<script id="manifest-data" type="application/json">${manifestText.trimEnd()}</script>`), 'utf8');
