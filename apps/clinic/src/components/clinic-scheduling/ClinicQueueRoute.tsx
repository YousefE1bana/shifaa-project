...
export function ClinicQueueRoute({ mode }: { mode: 'today' | 'queue' }) {
  ...
  const [sessionAal, setSessionAal] = useState<1 | 2 | null>(null);
    ...
    >
      ...
      {token ? (
        ...
        <main style={{ maxWidth: 560, marginInline: 'auto', padding: spacing.lg }}>
          <h1>{ar ? 'دخول موظف العيادة' : 'Clinic staff sign-in'}</h1>
          <p>
            {ar
