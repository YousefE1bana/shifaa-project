...
test('the root layout mounts the shared foreground session lifecycle', () => {
  ...
  assert.match(runtime, /addEventListener\('online', onlineListener\)/);
  assert.match(runtime, /const onlineListener = \(\) => void refresh\(\)/);
  assert.match(runtime, /removeEventListener\('online', onlineListener\)/);
...
test('the patient profile exposes both canonical logout scopes through the session controller', () => {
  const profile = fs.readFileSync(new URL('../app/profile.tsx', import.meta.url), 'utf8');
  assert.match(profile, /patientSessionRuntime\.controller\.logout/);
  assert.match(profile, /logout\(false\)/);
  assert.match(profile, /logout\(true\)/);
  assert.match(profile, /security\.session\.logoutCurrent/);
  assert.match(profile, /security\.session\.logoutAll/);
