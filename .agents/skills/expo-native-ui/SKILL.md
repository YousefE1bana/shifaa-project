...
> SHIFAA overlay: Consult `shifaa-ui-governor` before this skill. Do not apply the Apple-first palette, semantic colors, glass/blur, generic animation, font, spacing, radius, shadow, or navigation opinions below where they conflict with SHIFAA. Use SHIFAA semantic tokens, IBM Plex Sans Arabic/Inter, Arabic-first RTL plus English parity, contracted breakpoints/states, WCAG 2.2 AA, reduced motion, and zero decorative motion on safety-critical surfaces. Do not submit Expo feedback or install/change packages without explicit task authorization.
