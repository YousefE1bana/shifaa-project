...
> SHIFAA overlay: Consult `shifaa-project-guardrails` first. Use only the generated SHIFAA API client and catalogued Core API operations; never call Supabase domain tables, vendors, AI, or invented endpoints directly. Preserve RFC 9457 errors, idempotency, secure session handling, no-store/PHI rules, stale/offline UI contracts, and the pinned dependency stack. Do not submit Expo feedback or add packages without explicit task authorization.
