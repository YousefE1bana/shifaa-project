...
> SHIFAA overlay: Consult `shifaa-project-guardrails` and the Architecture plus Data/RLS contracts first. SHIFAA uses self-hosted Supabase infrastructure behind the Core API: clients do not receive domain-table access, normal user requests never use `service_role`, and domain tables remain private with forced RLS/default deny. Do not create MCP configuration, submit feedback, change dependencies, expose schemas, or apply remote changes unless the task explicitly authorizes it. Never send PHI, secrets, source, or production data externally.
