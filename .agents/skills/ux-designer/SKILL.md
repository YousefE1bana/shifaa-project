...
> SHIFAA overlay: Consult `shifaa-ui-governor` first. Where any generic metric, navigation pattern, timing, notification treatment, visual style, or interaction advice below differs from `docs/design/SHIFAA-UI-Contract.md`, use the SHIFAA contract. Do not transmit SHIFAA source, data, or feedback externally without explicit authorization.
