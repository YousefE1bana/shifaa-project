...
> SHIFAA overlay: Apply this as a second-pass quality gate after test or verification-code changes. `shifaa-project-guardrails`, approved contracts, feature acceptance criteria, forced-RLS negatives, and repository test conventions outrank generic testing advice. SpecKit remains the only lifecycle. Treat intentional real-stack, migration, bilingual, accessibility, idempotency, audit/outbox, and safety coverage as contracted behavior rather than test bloat; do not redesign architecture to make tests simpler.
