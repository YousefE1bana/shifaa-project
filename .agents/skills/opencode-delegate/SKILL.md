...
> SHIFAA overlay: Delegation is subordinate to `shifaa-project-guardrails`, `shifaa-ui-governor` for UI work, current SpecKit artifacts, and Issue/task acceptance criteria. Give the implementer a self-contained bounded brief; it inherits no unstated context and may not change requirements, invent APIs/data contracts, alter architecture, bypass `OPEN-*` gates, or introduce PHI/secrets. It must not commit, push, merge, close Issues, or clean branches. The parent owns briefing, diff review, applicable guard reviews, repository verification, commit, push, and the PR lifecycle. For migrations, RLS, auth, clinical safety, compliance, or API contracts, delegated output is evidence to review—not proof of correctness. A fresh run requires a human-approved authenticated provider/model; never invent or select a potentially billed model.
