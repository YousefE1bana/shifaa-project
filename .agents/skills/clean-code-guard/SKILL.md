...
> SHIFAA overlay: Apply this as a second-pass quality gate after production-code changes. `shifaa-project-guardrails`, approved contracts, the current feature specification, and repository conventions outrank generic clean-code advice. SpecKit remains the only lifecycle. Distinguish objective defects from deliberate SHIFAA choices, and do not redesign architecture, APIs, data models, authorization, RLS, safety behavior, or open-gate handling merely to satisfy a generic principle.
