...
> SHIFAA overlay: Apply this as a second-pass quality gate after documentation or governance changes. Repository authority documents, approved feature artifacts, and verified evidence outrank generic documentation advice; SpecKit remains the only lifecycle. Distinguish false or unverifiable claims from intentional SHIFAA contract language. Do not rewrite architecture, API, data/RLS, UI, legal, clinical, or `OPEN-*` decisions while trying to make documentation more generic or concise.
