...
const acceptedModes = new Set(['all', 'schema', 'chain']);
...
async function runSchemaMode() {
  ...
  try {
    ...
    try {
      ...
      `;
      assert.equal(childUniqueIndexes.length, 3, 'each completed fixture partition needs a chain unique index');
      ...
      for (const requiredIndex of requiredIndexes) assert(indexNames.has(requiredIndex), requiredIndex);
    ...
    try {
      ...
      await Promise.all([
        ...Array.from({ length: 16 }, (_, index) =>
          appendEvent('2026-05-15T10:00:00Z', index + 1),
        ),
