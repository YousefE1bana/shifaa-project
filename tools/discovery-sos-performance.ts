...
async function main() {
  ...
  const people = await owner<
    { id: string }[]
  >`select ${owner.unsafe(uuidExpression(run, 'person'))} id from generate_series(1,${totalSyntheticSubjects}::integer) i`;

  const patients = await owner<
    { id: string }[]
  >`select ${owner.unsafe(uuidExpression(run, 'patient'))} id from generate_series(1,${totalSyntheticSubjects}::integer) i`;
  ...
  await owner.begin(async (sql) => {
    await sql.unsafe(`
      ...
      from generate_series(1,${totalSyntheticSubjects}) i;
  ...
  try {
    ...
    const createIncident = async (
      ...
      const response = await harness.app.inject({
        ...
        remoteAddress: syntheticRemoteAddress(participantIndex),
        headers: {
          authorization: `Bearer synthetic-person:${personId}`,
          'x-shifaa-patient-context': patientId,
          'x-purpose': 'emergency_care',
          'idempotency-key': `sos-perf-${run}-${phase}-${participantIndex}`,
        },
        payload: {
          managed_patient_id: patientId,
          coordinates: {
            latitude: 30.1005 + participantIndex * 0.0001,
            longitude: 31.2005 + participantIndex * 0.0001,
          },
      ...
      return {
        ...
        incidentId: response.json().incident.incident_id as string,
      };
    };

    const warmupPeople = people.slice(0, mutationWarmupSamples);
    const warmupPatients = patients.slice(0, mutationWarmupSamples);
    await Promise.all(
      warmupPeople.map(({ id: personId }, index) =>
        createIncident(personId, warmupPatients[index]!.id, index, 'warmup'),
      ),
    );
    ...
    const mutations = await Promise.all(
      measuredPeople.map(({ id: personId }, index) => {
        const participantIndex = index + mutationWarmupSamples;
        return createIncident(personId, measuredPatients[index]!.id, participantIndex, 'measured');
      }),
    );
    ...
    const readTimes = await Promise.all(
      measuredPeople.map(async ({ id: personId }, index) => {
        const participantIndex = index + mutationWarmupSamples;
        const started = performance.now();
        ...
        const incRes = await harness.app.inject({
          ...
          headers: {
            authorization: `Bearer synthetic-person:${personId}`,
            'x-shifaa-patient-context': measuredPatients[index]!.id,
            'x-purpose': 'emergency_care',
          },
        });
    ...
    const evidence = {
      ...
      samples: {
        sessions: samples,
        warmup_sessions: mutationWarmupSamples,
        facilities: 6,
        sos_incidents: samples,
        warmup_sos_incidents: mutationWarmupSamples,
        worker_claims: samples,
        warmup_worker_claims: mutationWarmupSamples,
      },
      measurement_profile: {
    ...
    await owner.begin(async (sql) => {
      ...
      await sql`delete from platform.event_receipts where consumer = 'discovery-sos-contact-worker' or event_id in (select id from platform.outbox_events where aggregate_id in (select ${sql.unsafe(uuidExpression(run, 'patient'))} from generate_series(1,${totalSyntheticSubjects}::integer)i) or aggregate_type = 'sos-contact')`;
      await sql`delete from platform.notification_delivery_attempts where source_event_id in (select id from platform.outbox_events where aggregate_id in (select ${sql.unsafe(uuidExpression(run, 'patient'))} from generate_series(1,${totalSyntheticSubjects}::integer)i) or aggregate_type = 'sos-contact')`;
      ...
      await sql`delete from identity.callback_contact_verifications where person_id in (select ${sql.unsafe(uuidExpression(run, 'person'))} from generate_series(1,${totalSyntheticSubjects}::integer)i)`;
      await sql`delete from identity.patients where id in (select ${sql.unsafe(uuidExpression(run, 'patient'))} from generate_series(1,${totalSyntheticSubjects}::integer)i)`;
      await sql`delete from identity.people where id in (select ${sql.unsafe(uuidExpression(run, 'person'))} from generate_series(1,${totalSyntheticSubjects}::integer)i)`;
