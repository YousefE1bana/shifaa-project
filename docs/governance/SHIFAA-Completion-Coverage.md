...
5. **Context-message attachment intake:** resolved for Feature 010 by the dated Product Owner/Architecture reconciliation: `sendContextMessage` is body-only, rejects any `attachment` property including null, and persists a null `trust.messages.attachment`. There is no chat upload-intent operation or attachment UI. Future attachment enablement requires separate governance; this decision does not enable it.
6. **UI route/composition gaps:** the API Catalog defines disability review, pharmacy-director/catalog administration, finance review, subject-readable admission, ordering-clinician critical-result worklists/acknowledgement, facility complaint handling, and authorized admin AI-run review without exact corresponding routes/compositions in the UI Contract. Feature 010 patient `/records` owns pending referral review/acceptance and patient chat is assigned only to existing `/encounters/:id` for a linked active appointment context; clinic `/referrals` is clinician creation/tracking only and `/messages` remains context-bound. Its 87-state/408-reference TEST-ONLY functional `OPEN-UX-001` baseline was approved by Yousef Osama as Product Owner + Acting Design Lead for Feature 010 only on 2026-09-26 (manifest SHA-256 `18e4471d686990e797e8a5e370a20ceda7ea823020e3f84fdea0e03a65394310`). `OPEN-UX-002` formal visual criteria remain; other UI features retain their own `OPEN-UX-001` decisions. Other listed route/composition gaps remain unresolved and may not be guessed by later specs.
