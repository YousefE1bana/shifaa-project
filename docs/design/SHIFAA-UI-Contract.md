...
All UI code MUST use this file plus `packages/design-system`. No application may define its own color, radius, spacing, type scale, shadow, focus ring, breakpoint, toast, dialog, or safety-alert anatomy. Exact screen composition/reference images remain unapproved program-wide except for individually recorded decisions such as Feature 010's TEST-ONLY functional baseline. This document makes implementation deterministic at the system/component/route-state level but does not claim pixel-identical screens or final visual identity from that scoped approval; `OPEN-UX-001/002` retain their effects for other UI and formal visual verification.
...
| `/records`, `/encounters/:id`, `/prescriptions/:id`       | released record projections; Feature 010 pending referral list/preview/acceptance on `/records`; appointment-context message composition only within `/encounters/:id` | not fully checked, warning, cancelled/superseded; referral authority/slot conflict; context ended, participant removed, reconnect/stale/last updated on the encounter message composition |
...
Feature 008 realizes admin `/dashboard` and `/audit` with Arabic `ar-EG` RTL and English `en-EG` LTR parity. Dashboard states include loading, inactive-policy gated, empty, suppressed, stale/offline, error, and released test-only cells; canonical `metrics: []` displays no aggregate cells or exact counts. The audit workspace includes AAL2, purpose, permission, loading, empty, filtered list, redacted detail, export queued/retrying/failed/dead-letter/proven, stale/offline, and error states without raw metadata, suppressed/released exact counts, or a client-side offline export queue. Export updates come only from authoritative list/detail reconciliation; Feature 008 adds no polling/status operation.

Feature 009 realizes patient `/discover`, `/doctors/:id`, `/appointments/new`, `/appointments/:id` and clinic `/today`, `/queue`, `/schedule`, `/appointments/:id` for the existing 18-operation boundary. The same doctor search and appointment data is presented in Arabic RTL and English LTR, with the server-owned EGP fee and `cash_on_arrival` visible before confirmation. Clinic schedule editing covers recurrence, validity, `blocked`/`added` exceptions, overlap, delay supersession, and absence affected count with destructive confirmation; absence suggestions have no hold. Queue actions show entry and scope version conflicts and do not infer appointment state transitions. Both apps distinguish loading, empty, permission denial, stale, offline, error, conflict, and success; offline mutations are blocked. Destructive dialogs restore focus after cancellation, and status announcements use the shared polite/assertive pattern. The 492 actual route/state/locale/viewport captures and manual structural observations are recorded in the Feature 009 synthetic evidence manifest. They do not establish pixel identity, formal NVDA/TalkBack coverage, reference-device acceptance, or `OPEN-UX-002` closure; approved reference PNGs remain immutable.

Both routes preserve logical keyboard order, visible focus and focus return, semantic names/status announcements, 44-by-44 targets, reduced motion, forced colors, 200% text, and 400% reflow; identifiers, codes, hashes, cursors, and RFC 3339 values are bidi-isolated where rendered. Recorded screenshots and browser semantic checks use synthetic graduation data and remain informative engineering evidence. `OPEN-UX-001/002`, `OPEN-TECH-003`, and `OPEN-PRODUCT-001` still prevent pixel-identity, reference-device/assistive-technology certification, formal visual acceptance, or UAT approval.

Feature 010 places patient message composition only inside existing `/encounters/:id`, for that encounter's linked appointment while the encounter is `open` and appointment is `in_consultation`. The subject patient may participate during that eligible lifecycle; workforce chat requires an active encounter-participant interval, and GUA/DEL have no Feature 010 chat access. Clinic `/messages` displays only an explicitly selected eligible appointment context. There is no patient inbox, new route, pre-encounter chat, general consultation channel, attachment control, or offline message queue. Patient/clinic message views reauthorize and reconcile through authoritative REST after reconnect and expose stale and last-updated states; workforce interval ending or encounter completion removes read/send access immediately. Patient `/records` owns the role-projected pending referral list, exact `reason_summary`/optional `encounter_type` preview and explicit PAT/GUA/DEL acceptance; GUA requires current active approved guardianship, while DEL requires both current `record.view` and `appointment.manage`, rechecked at each read/action. Clinic `/referrals` provides clinician creation/tracking only, with no clinic acceptance action. The views distinguish `private` from `patient_visible` signed notes and never disclose a pending referral to its target. Completion UI asks the responsible clinician for a nonblank authored summary and explicit structural confirmation without claiming clinical sufficiency. On 2026-09-26 Yousef Osama, as Product Owner + Acting Design Lead for Feature 010 only, approved the exact six TEST-ONLY functional source compositions and 87-state/408-reference baseline at manifest SHA-256 `18e4471d686990e797e8a5e370a20ceda7ea823020e3f84fdea0e03a65394310`. This satisfies Feature 010's functional `OPEN-UX-001` affected-UI prerequisite; `OPEN-UX-002`, live accessibility evidence and final visual identity remain separate.
...
Required viewports are 360×800 and 412×915 for patient compact, 768×1024 medium, and 1440×900 wide staff. Tests run `ar-EG` and `en-EG`, light mode, plus dark mode for shared primitives. Fonts and animations are stabilized. Baselines are accepted only by Product Owner + Design Lead and are identified by design node/version plus screenshot SHA-256. Feature 010 has a separately approved TEST-ONLY functional baseline; its static snapshots establish the reviewed composition reference, not application behavior or formal `OPEN-UX-002` visual acceptance. For other affected UI, snapshots remain informative until their `OPEN-UX-001` decision is recorded.
