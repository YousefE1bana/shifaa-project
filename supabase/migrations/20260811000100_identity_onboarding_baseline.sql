-- Generated mechanically from canonical 001 schema SHA256 3605e264652c1d45e3c10e054dce0b8649c8a92d1e61d654938d22c5030313a9
-- and RLS SHA256 47171d29449c323f129ed21a6c1c79589aae108ff1e43e5bc31e8d28866d7571. Keep sources synchronized; do not edit this copy alone.
...
CREATE OR REPLACE FUNCTION platform.register_identity_onboarding(
  p_user_id uuid,
  p_handle text,
  p_locale text
) RETURNS TABLE(person_id uuid, patient_id uuid, relationship_id uuid, version integer)
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = pg_catalog AS $$
DECLARE v_person_id uuid; v_patient_id uuid; v_relationship_id uuid; v_version integer;
BEGIN
  INSERT INTO identity.people(user_id, email_normalized, preferred_locale)
  VALUES (p_user_id, lower(p_handle), p_locale)
  ON CONFLICT (user_id) DO UPDATE SET user_id = EXCLUDED.user_id
  RETURNING id, identity.people.version INTO v_person_id, v_version;

  INSERT INTO identity.patients(person_id, medical_record_number)
  VALUES (v_person_id, 'SYN-' || upper(substr(replace(v_person_id::text, '-', ''), 1, 16)))
  ON CONFLICT ON CONSTRAINT patients_person_id_key DO UPDATE SET person_id = EXCLUDED.person_id
  RETURNING id INTO v_patient_id;

  INSERT INTO identity.care_relationships(subject_patient_id, actor_person_id, relationship_type)
  VALUES (v_patient_id, v_person_id, 'self')
  ON CONFLICT (subject_patient_id) WHERE relationship_type = 'self' AND status = 'active'
  DO UPDATE SET actor_person_id = EXCLUDED.actor_person_id
  RETURNING id INTO v_relationship_id;

  RETURN QUERY SELECT v_person_id, v_patient_id, v_relationship_id, v_version;
END
$$;

CREATE OR REPLACE FUNCTION platform.resolve_person_id(p_user_id uuid) RETURNS uuid
LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = pg_catalog AS $$
  SELECT id FROM identity.people WHERE user_id = p_user_id
$$;

REVOKE ALL ON FUNCTION platform.register_identity_onboarding(uuid, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION platform.resolve_person_id(uuid) FROM PUBLIC;
...
VALUES ('identity-onboarding-synthetic', 'SHIFAA Product Owner', 'SHIFAA synthetic environment', ARRAY['identity_proofing', 'care_updates'], ARRAY['identity', 'contact'], ARRAY['local-api'], 'identity-proofing', 'synthetic-engineering-only', 'baseline-v2.1.0-synthetic', 'active')
ON CONFLICT DO NOTHING;
