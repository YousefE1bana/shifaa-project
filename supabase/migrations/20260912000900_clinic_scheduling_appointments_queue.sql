...
CREATE TABLE clinical.queue_entries (
  ...
  reorder_reason text CHECK (reorder_reason IS NULL OR (length(reorder_reason) BETWEEN 1 AND 500 AND reorder_reason !~ '^[[:space:]]*$' AND reorder_reason !~ '[[:cntrl:]]')),
...
CREATE FUNCTION clinical.reorder_queue_entry_v1(p_entry_id uuid,p_expected_version integer,p_expected_queue_version integer,p_target_position integer,p_reason text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path='' AS $$
...
BEGIN
  IF p_target_position IS NULL OR p_target_position<1 OR p_reason IS NULL OR length(p_reason) NOT BETWEEN 1 AND 500 OR p_reason ~ '^[[:space:]]*$' OR p_reason ~ '[[:cntrl:]]' THEN RAISE EXCEPTION 'invalid reorder request' USING ERRCODE='22023'; END IF;
  SELECT * INTO entry FROM clinical.queue_entries WHERE id=p_entry_id FOR UPDATE;
  ...
  IF scope_version IS DISTINCT FROM p_expected_queue_version THEN RAISE EXCEPTION 'queue scope version is stale' USING ERRCODE='40001'; END IF;
  SELECT count(*)::integer INTO max_position FROM clinical.queue_entries q WHERE q.queue_scope_id=scope_id AND q.state='waiting';
  IF p_target_position>max_position THEN RAISE EXCEPTION 'target position is outside waiting queue' USING ERRCODE='22023'; END IF;
  PERFORM pg_catalog.set_config('shifaa.clinic_internal_transition','allowed',true);
