...
>
> **Approved on:** 2-Sep-2026
>
> **Approval version:** v2.1.3
>
> **Approval record:** [`OPEN-PRIV-001 decision package v1.0.0`](./specs/008-audit-admin-aggregates-observability/decisions/OPEN-PRIV-001-reidentification-risk-decision-package.md)
>
> **Last verified:** 2026-09-02 (Africa/Cairo)
>
> **Companion product source:** [`shifaa-prd.md`](./shifaa-prd.md)

## 0. Operating rules

This document contains the approved settled architecture, its reasoning, the immutable Constitution text, implementation sequence, SpecKit lifecycle, and the synchronized open-item register. It became the implementation baseline when Product Owner Yousef Osama approved v2.1.0 on 9-Aug-2026, was amended to v2.1.1 on 25-Aug-2026 to close `OPEN-LEGAL-006`, was amended to v2.1.2 on 25-Aug-2026 to close `OPEN-TEAM-001` and development-stage `OPEN-SEC-001` for Feature 007, and was amended to v2.1.3 on 2-Sep-2026 to close `OPEN-PRIV-001` for Feature-008 graduation engineering. Artifact digests are recorded in the linked approval records. “Settled” means the team implements it unless a dated ADR and Product Owner approval amend it. Legal and clinical facts still require the evidence named in the compliance register.
...
The canonical [graduation roadmap](docs/governance/SHIFAA-Remaining-Specs-Roadmap.md) records completed Features 001-006, preserves the 007-026 feature sequence, and defines Feature 026 as integrated feature-complete engineering closure. A distinct **POST-026 PROJECT-WIDE POLISH PHASE** follows for final cross-app visual refinement and then project-wide visual, accessibility, and regression reverification. Polish is not Feature 027 and adds no FR/NFR/API/data/business behavior; controlled replacement of TEST-ONLY functional visual baselines cannot alter approved functional, security/privacy, or accessibility contracts. This phase does not waive the release-signature set or any applicable `OPEN-*` evidence.
...
| OPEN-TEAM-001    | 2026-08-25 by Product Owner approval                                                                                                         | **CLOSED:** Yousef Osama is Product Owner, Team Lead, Architecture Lead, SpecKit/Governance Owner, and current pre-implementation engineering/security decision authority. Mostafa Ali is implementation Security Lead; Diaa Eldin Assem owns Backend/Core API implementation; Ibrahim Saeid owns Data/PostgreSQL/RLS implementation; Amira Saad owns QA/testing/evidence; Ziad Wael owns Frontend/UI/A11y/integration; Dr Asmaa Hekal is Academic Supervisor; TA Mahmoud Ghalwash is Academic Reviewer/TA.                                                                                                | Yousef solely owns `specify -> clarify -> plan -> tasks -> analyze -> taskstoissues`; team members implement assigned work later under approved artifacts and do not individually approve them. Assignment acknowledgement/contact rotation is recorded when implementation activates, not as an independent `SPEC_APPROVED` signature.                                                                                                                                                                                                                  | This assignment grants no runtime/repository permission and does not replace legal/DPO, clinical, security-testing, vendor, UX, technology, or production-release approval. Implementation still requires Yousef's explicit authorization.                                                                                                        |
| OPEN-SEC-001     | 2026-08-25 by Product Owner/Architecture approval                                                                                            | **CLOSED for Feature-007 specification/development:** 15-minute access JWT; configured 23h45m absolute and 45m inactivity timeouts bounding effective access to 24h/60m; rotating refresh tokens with the documented 10-second reuse interval and whole-family/session revocation outside benign exceptions; workforce/admin AAL2 and exact five-minute qualifying-factor freshness; TOTP supported, passkeys disabled until proven; recovery cannot downgrade MFA.                                                                                                                                        | The v2.1.2 amendment and approved security memo freeze foreground-only refresh, current/all-session revocation, native-session validation, fail-closed Auth dependency behavior, serialized factor mutation, uniform recovery, restricted recovery sessions, deterministic fake-clock/replay/race tests, and secret-safe cookie/native storage. No shadow session state, direct Auth-table mutation, new endpoint, or service-role online path is authorized.                                                                                            | Policy approval does not authorize implementation or production deployment. Exact Supabase compatibility/DDL/RLS, generated contracts, ASVS/API abuse testing, live security evidence, production secrets/KMS, and release approval remain required under `OPEN-TECH-002`, `OPEN-TECH-003`, `NFR-SEC-007`, and unchanged production gates.        |
| OPEN-PRIV-001    | 2026-09-02 by Yousef Osama under Product Owner / Architecture Lead, Security Lead, Data Lead, and Project DPO / Privacy Decision Owner roles | **CLOSED for Feature-008 graduation engineering:** `k=11`; suppress 0–10; count distinct protected subjects; enforce the approved dimensions/combinations, parent-total complementary suppression, linked-release checks, configuration contract, and deterministic vectors. `metrics: []` activates no metric and does not block closure or planning.                                                                                                                                                                                                                                                     | Approved decision package v1.0.0 records the re-identification assessment, moderate residual-risk acceptance for the access-controlled synthetic graduation scope, and four role-specific approval rows. Individual metrics and status mappings remain fail-closed until later approved configuration.                                                                                                                                                                                                                                                   | Policy-level changes require a new approved package. This closure does not authorize implementation, production PHI, statutory DPO claims, public release, or any metric without its approved configuration; all later-stage legal, security-testing, technology, UX, UAT, and release gates remain.                                              |
...
| Date       | Version | Change                                                                                                                                                                                                                                                                                                                              |
| ---------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-02 | 2.1.3   | Closed `OPEN-PRIV-001` for Feature-008 graduation engineering through approved package v1.0.0; fixed `k=11`, 0–10 suppression, distinct-subject and differencing controls, preserved `metrics: []` as non-blocking inactive state, and retained all per-metric, implementation, production, UX, technology, UAT, and release gates. |
| 2026-08-25 | 2.1.2   | Closed `OPEN-TEAM-001` under the Product Owner-approved SHIFAA operating model and closed `OPEN-SEC-001` for Feature-007 specification/development with exact session, MFA, recovery, revocation, and deterministic-test policy; implementation and production gates remain intact.                                                 |
